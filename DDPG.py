...
class Actor(nn.Module):
    ...
    def compute_power(self, a):
        # Normalize the power
        W_real = a[:, :self.M ** 2].cpu().data.numpy()
        W_imag = a[:, self.M ** 2:2 * self.M ** 2].cpu().data.numpy()

        W = W_real.reshape(W_real.shape[0], self.M, self.K) + 1j * W_imag.reshape(W_imag.shape[0], self.M, self.K)

        WW_H = np.matmul(W, np.transpose(W.conj(), (0, 2, 1)))
        #print(WW_H.shape)
        current_power_t = torch.sqrt(torch.from_numpy(np.real(np.trace(WW_H, axis1=1, axis2=2)))).reshape(-1, 1).to(self.device)
        #print(current_power_t.shape)
        return current_power_t

    def compute_phase(self, a):
        # Normalize the phase matrix
        Phi_real = a[:, -2 * self.N:-self.N].cpu().data.numpy()
        Phi_imag = a[:, -self.N:].cpu().data.numpy()
        
        Phi_cal = Phi_real.reshape(Phi_real.shape[0],self.N) + 1j * Phi_imag.reshape(Phi_imag.shape[0],self.N)
        #print(Phi_cal.shape)
        Phi = torch.abs(torch.from_numpy(Phi_cal)).to(self.device)
        #print(Phi.shape)
        return Phi
    ...
    def forward(self, state):
        ...
        Phi = self.compute_phase(a.detach()).repeat(1,2)

        division_term = torch.cat([current_power_t,Phi], dim=1)
...
class DDPG(object):
    ...
    def select_action(self, state):
        ...
        action = self.actor(state).data.numpy().flatten().reshape(1, -1)
